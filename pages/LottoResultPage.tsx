    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col relative overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 z-10 space-y-8 w-full max-w-md mx-auto">
        ...
        <div className="w-full bg-white/5 backdrop-blur-xl rounded-2xl p-6 border border-white/10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-wrap justify-center gap-3 relative z-10">
              {numbers.map((num: number, idx: number) => (
                <div key={idx} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-b from-red-500 to-red-700 flex items-center justify-center shadow-[0_4px_6px_rgba(0,0,0,0.3)] border border-red-400 text-white font-bold text-lg sm:text-xl animate-pulse-slow" style={{ animationDelay: `${idx * 0.15}s` }}>
                  {num}
                </div>
              ))}
        ...
        <div className="w-full space-y-3 pt-2">
          <button onClick={handleVIP} className="w-full py-4 bg-gradient-to-r from-yellow-400 via-orange-400 to-yellow-400 bg-[length:200%_auto] animate-shimmer rounded-xl font-bold text-slate-900 shadow-[0_0_20px_rgba(250,204,21,0.4)] flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all">
            <Crown className="w-5 h-5" />
            <span>Xem Gợi Ý Số VIP Theo Tuổi</span>
          </button>
          <button onClick={handleSave} className="w-full py-3 bg-slate-800/50 border border-slate-600 rounded-xl font-semibold text-slate-200 hover:bg-slate-700 hover:border-slate-500 transition flex items-center justify-center gap-2">
            <Save className="w-5 h-5 text-slate-400" />
            <span>Lưu Bộ Số Này</span>
          </button>
          <button onClick={handleRetry} className="w-full py-2 text-slate-500 text-sm hover:text-white transition flex items-center justify-center gap-1">
