        ...
        <main className="flex flex-col gap-4 p-5 w-full animate-fade-in">
            ...
            <section className="grid grid-cols-3 gap-3">
                {ZODIACS.map((animal) => (
                    ...
                    >
                        <div className={`w-20 h-20 rounded-full ${animal.id === 'monkey' ? 'bg-yellow-50 border-yellow-100' : 'bg-red-50 border-red-100'} border-2 flex items-center justify-center overflow-hidden group-hover:scale-105 transition-transform bg-gradient-to-br from-white to-gray-50`}>
                            {animal.image ? (
                                <img 
                                    alt={animal.name} 
                                    className="w-16 h-16 object-contain p-1 drop-shadow-sm" 
                                    src={animal.image} 
                                    onError={(e) => {
